import $ from 'jquery';
import 'normalize.css';
import 'ion-rangeslider/css/ion.rangeSlider.css'
import 'ion-rangeslider/js/ion.rangeSlider';
import 'air-datepicker/dist/js/datepicker.js';
import 'air-datepicker/dist/css/datepicker.css';


import './assets/scss/fonts.scss';

import './modules/bullet-list/bullet-list.js';
import './modules/button/button.js';
import './modules/card-room/card-room.js';
import './modules/checkbox/checkbox.js';
import './modules/checkbox-list/checkbox-list.js';
import './modules/comment/comment.js';
import './modules/diagram/diagram.js';
import './modules/dropdown/dropdown.js';
import './modules/dropdown-date/dropdown-date.js';
import './modules/field/field.js';
import './modules/font-example/font-example.js';
import './modules/footer/footer.js';
import './modules/form-login/form-login.js';
import './modules/form-registration/form-registration.js';
import './modules/form-search/form-search.js';
import './modules/header/header.js';
import './modules/icon/icon.js';
import './modules/icon-with-text/icon-with-text.js';
import './modules/icon-with-text-list/icon-text-list.js';
import './modules/like-button/like-button.js';
import './modules/logo/logo.js';
import './modules/navigation/navigation.js';
import './modules/pagination/pagination.js';
import './modules/palette/palette.js';
import './modules/radio/radio-button.js';
import './modules/range-slider/range-slider.js';
import './modules/rate-button/rate-button.js';
import './modules/socials/socials.js';
import './modules/subscribe/subscribe.js';
import './modules/toggle/toggle.js';
import './modules/room-reservation/room-reservation.js'


import './pages/ui-kit/colors-and-types/colors-and-types.js';
import './pages/ui-kit/form-elements/form-elements.js'
import './pages/ui-kit/cards/cards.js';
import './pages/ui-kit/headers-and-footers/headers-and-footers.js';
//по мере работы подключить оставшиеся странички